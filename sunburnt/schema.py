...
from ._compat import str_type, text_type, long_type, reduce_func
...
def solr_point_factory(dimension):
    ...
    class solr_point(object):
        ...
        def __repr__(self):
            return "solr_point(%s)" % text_type(self)
...
class SolrUnicodeField(SolrField):
    def from_user_data(self, value):
        if isinstance(value, SolrString):
            ...
            return WildcardString(text_type(value))
...
class SolrBinaryField(SolrField):
    ...
    def to_solr(self, value):
        return text_type(value.encode('base64'))
...
class SolrPointField(SolrField):
    ...
    def to_solr(self, v):
        return text_type(self.value_class(v))
...
class SolrSchema(object):
    ...
    def check_fields(self, field_names, required_atts=None):
        if isinstance(field_names, str_type):
...
class SolrUpdate(object):
    ...
    def doc(self, doc):
        ...
        if not doc:
            ...
            return self.DOC(*reduce_func(operator.add,
...
class SolrDelete(object):
    ...
    def doc_id_from_doc(self, doc):
        ...
        # arbitrary object.
        if isinstance(doc, (str_type, int, text_type, float)):
    ...
    def delete_queries(self, queries):
        if isinstance(queries, text_type) or not hasattr(queries, "__iter__"):
            queries = [queries]
        return [self.QUERY(text_type(query)) for query in queries]
